...
import { ImageAnalysis, Message, MessageSender, GlobalChatMessage as GlobalChatMessageType } from './types';
...
// MainDashboard: Handles navigation between Home, Image Analyzer, Chat, and Global Chat
const MainDashboard: React.FC<{ user: User }> = ({ user }) => {
  const [view, setView] = useState<'home' | 'image-analyzer' | 'chat' | 'global-chat'>('home');
...
const HomePage: React.FC<{ user: User; setView: (view: 'image-analyzer' | 'chat' | 'global-chat') => void }> = ({ user, setView }) => {
  return (
    <>
